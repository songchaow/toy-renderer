...
static Image* default_metalRough = new Image(R8G8B8(128, 12, 0), false, 0.f, false);
...
ImageTexture PBRMaterial::lut;

PBRMaterial defaultMaterial(default_color, default_metalRough, default_normal_color);

void PBRMaterial::loadLUTFile() {
      ;
      Image* lutImage = new Image("texture/lut.png", Image::RGBSpectrum, false, false);
      lutImage->setConvertFromsRGB(false);
      lutImage->setRequestNumChannel(2);
      lutImage->Load();
      lut.resetImage(lutImage);
}

void PBRMaterial::glLoadLUT() {
      lut.load();
}