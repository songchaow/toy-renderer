...
REM How to use:
REM BUILD 64-bit:
REM    "msvc_build.bat debug" builds 64-bit Debug version of ToyTracer without optimizations
REM    "msvc_build.bat relwithdebinfo" builds optimized 64-bit version of ToyTracer with almost all debug symbols
REM    "msvc_build.bat release" builds fully optimized 64-bit version of ToyTracer without command line output
REM
REM BUILD 32-bit:
REM    "msvc_build.bat debug 32" builds 32-bit Debug version of ToyTracer
REM    "msvc_build.bat relwithdebinfo 32" builds 32-bit RelWithDebInfo version of ToyTracer
REM    "msvc_build.bat release 32" builds 32-bit Release version of ToyTracer
REM
REM INSTALL 64-bit:
...
REM
REM PACKAGE:
REM    "msvc_build.bat package" pack the installer for already built and installed 64-bit Release build (msvc.build_x64/ToyTracer-*.msi)
REM    "msvc_build.bat package 32" pack the installer for already built and installed 32-bit Release build (msvc.build_x86/ToyTracer-*.msi)
REM
REM CLEAN:
...
IF /I "%1"=="revision" (
   echo revisionStep
   git rev-parse --short=7 HEAD > toytracer/revision.h
   GOTO :END
   )
...
REM -DCMAKE_BUILD_NUMBER=%BUILD_NUMBER% -DCMAKE_BUILD_AUTOUPDATE=%BUILD_AUTOUPDATE% %CRASH_REPORT_URL_OPT% are used for CI only
   ...
   echo Building ToyTracer...
   cd "%BUILD_FOLDER%" & cmake --build . --config %CONFIGURATION_STR% --target toytracer
   GOTO :END
   )

:INSTALL
   cd "%BUILD_FOLDER%"
   echo Installing ToyTracer files...
